...
class IQL(object):
    def __init__(
        ...
        self.qf_optimizer = torch.optim.Adam(self.qf.parameters(), lr=lr)
    ...
    def update_v(self, states, actions, log_to_wb=False):
        with torch.no_grad():
            q1, q2 = self.qf_target(states)  # 输出所有动作的Q值, q1.shape [256,6]

            actions_long = actions.long() #action.sahpe [256]
            # 用actions索引出对应的Q值
            q1 = q1.gather(1, actions_long.unsqueeze(1)) # [256,1]
            q2 = q2.gather(1, actions_long.unsqueeze(1))
            q = torch.minimum(q1, q2).detach() #[256,1]
            # q1, q2 = self.qf_target(states, actions)
            # q = torch.minimum(q1, q2).detach()

        v = self.vf(states) #v.shape [256,1]
    ...
    def update_q(self, states, actions, rewards, next_states, not_dones, log_to_wb=False):
        #torch.Size([256, 36]) torch.Size([256]) torch.Size([256, 1]) torch.Size([256, 36]) torch.Size([256, 1])
        with torch.no_grad():
            next_v = self.vf(next_states)
            target_q = (rewards + self.discount * not_dones * next_v).detach()
            #target_q [256,1]

        q1, q2 = self.qf(states)
