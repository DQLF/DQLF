...
from baseline_special.utils.constants import (
    ...
    M_IN_K, SMOOTH_PENALTY, VIDEO_BIT_RATE, HD_REWARD,CHUNK_TIL_VIDEO_END_CAP, RAND_RANGE, DEFAULT_QUALITY, TOTAL_VIDEO_CHUNK
)
...
CHUNK_COMBO_OPTIONS = np.array([combo for combo in itertools.product(
                range(8), repeat=MPC_FUTURE_CHUNK_COUNT)])
...
def calculate_jump_action_combo(br):
    all_combos = CHUNK_COMBO_OPTIONS # (8**5,5) combinations
...
def calculate_rebuffer(size_video_array, future_chunk_length, buffer_size, bit_rate, last_index, future_bandwidth, jump_action_combos, video_bit_rate):
    ...
    for full_combo in jump_action_combos:
        ...
        for position in range( 0, len( combo ) ):
            ...
            reward = (video_bit_rate[chunk_quality] / 1000.) - (REBUF_PENALTY *curr_rebuffer_time) - (SMOOTH_PENALTY * abs(video_bit_rate[chunk_quality] - video_bit_rate[last_quality]) / 1000.)
            reward_sum += reward
...
def run(args):
    assert args.model in ['pensieve','genet', 'merina','mpc', 'bba']
    ...
    cuda_start = torch.cuda.memory_allocated()

    if args.model in  ['pensieve','genet', ]:
        model = PENSIEVE
    elif args.model == 'mpc':
        model = MPC
    elif args.model == 'bba':
    ...
    net_env = env.Environment(all_cooked_time=all_cooked_time, all_cooked_bw=all_cooked_bw, all_file_names=all_file_names, all_mahimahi_ptrs=all_mahimahi_ptrs,
                              video_size_dir=video_size_dir, fixed=args.fixed_order, trace_num=trace_num,training=False)

    results_dir = os.path.join(cfg.results_dir, args.model, f'{args.test_trace}')
    ...
    with tf.compat.v1.Session() as sess:
        ...
        while True:  # serve video forever
            ...
            if model == MERINA:
                ...
                bit_rate = mpc(size_video_array, state, last_bit_rate, buffer_size, video_chunk_remain, video_bit_rate, past_errors,
                               past_bandwidth_ests, combo_dict)
            else:
                bit_rate = bba(buffer_size)
            t1=time.perf_counter()
            latences.append(t1-t0)
            if end_of_video:
                result_file.write('\n')
                result_file.close()
                time_stamp = 0
                trace_rewards = np.array(all_rewards[-TOTAL_VIDEO_CHUNK:])  # 当前视频的所有reward
                mean_reward = np.mean(trace_rewards)
                old_path = result_path
                new_path = f"{old_path}_reward_{mean_reward:.6f}"
                ...
                result_path = os.path.join(results_dir, f'result_sim_abr_{all_file_names[net_env.trace_idx]}')
                result_file = open(result_path, 'w')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--test-trace", help='', default='test')
    parser.add_argument('--video', help='name of testing videos (e.g., video1)', default='video1')
    parser.add_argument('--test-trace-num', type=int, help='number of traces for testing. if set to -1, use all traces in the trace dir.', default=-1)
