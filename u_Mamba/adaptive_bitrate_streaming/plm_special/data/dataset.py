...
class ExperienceDataset(Dataset):
    ...
    """
    def __init__(self, exp_pool, gamma=1., scale=10, max_length=20, sample_step=None) -> None:
        """
    ...
    def sample_batch(self, batch_size=1, batch_indices=None):
        ...
        batch_states, batch_actions, batch_returns, batch_timesteps, batch_traj_upbounds, batch_traj_rewards = [], [], [], [],[],[]
        for i in range(batch_size):
            states, actions, returns, timesteps, traj_upbounds, traj_rewards = self[batch_indices[i]]
            batch_states.append(states)
            batch_actions.append(actions)
            batch_returns.append(returns)
            batch_timesteps.append(timesteps)
            batch_traj_upbounds.append(traj_upbounds)
            batch_traj_rewards.append(traj_rewards)
        return batch_states, batch_actions, batch_returns, batch_timesteps, batch_traj_upbounds, batch_traj_rewards
    ...
    @property
    def traj_upbounds(self):
        return self.exp_pool.traj_upbounds
    
    @property
    def traj_rewards(self):
        return self.exp_pool.traj_rewards
    ...
    def __getitem__(self, index):
        ...
        return self.states[start:end], self.actions[start:end], self.returns[start:end], self.timesteps[start:end], self.traj_upbounds[start:end], self.traj_rewards[start:end]
