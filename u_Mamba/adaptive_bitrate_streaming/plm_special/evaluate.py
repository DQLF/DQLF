...
from baseline_special.utils.constants_whr import (TOTAL_VIDEO_CHUNK,
...
def evaluate_on_env(args, env_settings, model, target_return, max_ep_num=50, process_reward_fn=None, seed=0):
    ...
    with torch.no_grad():
        ...
        with tqdm(total=max_ep_num, desc='[Evaling]', unit='ep') as pbar:
            episodes_rward = 0
            while True:
                ...
                bit_rate = model.sample(state, target_return, timestep, training=False)
                timestep += 1
    ...
    eval_log.update({
        ...
        'episodes_reward': episodes_rward/max_ep_num/(TOTAL_VIDEO_CHUNK-1),
