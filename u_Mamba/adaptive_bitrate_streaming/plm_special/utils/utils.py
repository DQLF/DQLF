...
import random
import numpy as np
from baseline_special.utils.constants_whr import BITRATE_LEVELS,TOTAL_VIDEO_CHUNK
...
def process_batch(batch, device='cpu'):
    ...
    """
    states, actions, returns, timesteps, traj_upbounds, traj_rewards = batch
    ...
    traj_upbounds = torch.as_tensor(traj_upbounds, dtype=torch.float32, device=device).reshape(1, -1, 1)
    traj_rewards = torch.as_tensor(traj_rewards, dtype=torch.float32, device=device).reshape(1, -1, 1)/(TOTAL_VIDEO_CHUNK-1)

    return states, actions, returns, timesteps, labels, traj_upbounds, traj_rewards
